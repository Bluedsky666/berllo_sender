...
class EmailClientGUI:
    ...
    def create_config_frame(self):
        ...
        ttk.Checkbutton(config_frame, text="使用SSL", variable=self.ssl_var).grid(row=0, column=4, padx=5, pady=5)
        ttk.Label(config_frame, text="密钥:").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.key_var = tk.StringVar()
        ttk.Entry(config_frame, textvariable=self.key_var, width=40, show="*").grid(row=1, column=1, columnspan=3, padx=5, pady=5, sticky="we")
        self.test_button = ttk.Button(config_frame, text="测试连接", command=self.test_api_connection)
        self.test_button.grid(row=1, column=4, padx=10, pady=5)
        self.status_label = ttk.Label(config_frame, text="请先配置并测试连接", foreground="blue")
        self.status_label.grid(row=1, column=5, padx=5, pady=5, sticky="w")
    ...
    def _test_api_connection_thread(self):
        # ... (same as before, collapsed for brevity)
        try:
            self.api_client = ApiClient(host=self.host_var.get(), port=int(self.port_var.get()), key=self.key_var.get(), use_ssl=self.ssl_var.get())
    ...
    def _bulk_send_thread(self):
        ...
        self.root.after(0, self.status_summary_label.config, {"text": f"0 / {total_emails}"})
        try:
                ...
                if query_response.get("ret") == 200 and "data" in query_response and "result" in query_response["data"]:
                    for res_str in query_response["data"]["result"]:
                        try:
                            ...
                            if guid not in results: results[guid] = status_part; processed_count += 1
                        except ValueError: continue
                elapsed_time = time.time() - start_time
                eta_str = f" | ETA: {time.strftime('%H:%M:%S', time.gmtime(elapsed_time / processed_count * (len(guids_to_query) - processed_count)))}" if processed_count > 0 else ""
                self.root.after(0, self.progress_bar.config, {"value": processed_count})
                self.root.after(0, self.status_summary_label.config, {"text": f"{processed_count} / {len(guids_to_query)}{eta_str}"})
            self.root.after(0, messagebox.showinfo, "发送完成", f"全部 {len(guids_to_query)} 封邮件已处理完毕。")
        except Exception as e:
